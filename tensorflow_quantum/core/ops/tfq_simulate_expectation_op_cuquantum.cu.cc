...
class TfqSimulateExpectationOpGpuCpu : public tensorflow::OpKernel {
 public:
  explicit TfqSimulateExpectationOpGpuCpu(tensorflow::OpKernelConstruction* context)
    ...
    // Get the number of threads in SimulatorCuStateVec via attributes.
 ...
 private:
  ...
  void ComputeLarge(
    ...
    // Begin simulation.
    int largest_nq = 1;
    Simulator sim = Simulator(custatevec_handle);
    StateSpace ss = StateSpace(cublas_handle, custatevec_handle);
    auto sv = ss.Create(largest_nq);
    ss.SetStateZero(sv);
    auto scratch = ss.Create(largest_nq);
